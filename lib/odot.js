...
function prototizeEntries(entries, proto, Constructor) {
    var i, construct, entry, protoEntry; 
    for (i in entries) {
        if (entries.hasOwnProperty(i)) {
            ...
            // the entry prototype.
            entry = entries[i];
            protoEntry = new Constructor(i);  // Replaces original in db
            for (i in entry) {
...
function prototize(key, file) {
    ...
    proto.property =  function (name, calculator) {
    ...
    if (!prototyped) {
        prototizeEntries(entries, proto, construct);        
    }
}
